...
static void skipNonAlpha(FILE *fp);
...
readWords(FILE *fp)
{
	...
	if (ch == '\"')
	{
		...
		while (ch != '\"')
		{
			if (ch == EOF)
			{
				fprintf(stderr, "SCAN ERROR: attempt to read a BST String failed\n");
				fprintf(stderr, "no closing double quote\n");
				exit(6);
			}
			while (!isspace(ch) && ch != '\"')
			{
				...
				{
					++size;
					buffer = reallocateMsg(buffer, size, "readBstString");
				}
				if (isalpha(ch)) //Check is alpha
				{
					if (isupper(ch)) //If alpha is Up, change to low
						ch = tolower(ch);
					if (space == 1)//Go add space.
					{
						buffer[index] = ' '; //Get in to the buffer
						++index;
						if (index > size - 2)
						{
							++size;
					...
					space = 0;
					buffer[index] = ch; //Get in to the buffer
					//printf("%c\n",ch);
					++index;
				}
				ch = fgetc(fp);
				if (isspace(ch) && index !=0)
					space = 1;//Notice a space need be added.
				else if (ch == '\"')
					ungetc(ch,fp);
				if (ch == EOF)
				{
	...
	else 
	{
		while (!isspace(ch))
		{
			...
			{
				++size;
				buffer = reallocateMsg(buffer, size, "read Bst Token");
			}
			if (isalpha(ch)) //Check is alpha
			{
				if (isupper(ch)) //If alpha is Up, change to low
...
static void
skipNonAlpha(FILE *fp)
{
	int ch;

	/* read chars until a alphabetic letter is encountered */

	while ((ch = fgetc(fp)) != EOF && !isalpha(ch) && ch != '\"')
		continue;

	/* a alphabetic letter character got us out of the loop, so push it back */
